 ...
 * and on release it flies to the nearest resting place. `ToolbarCorner.ts`
 * owns the nine-place vocabulary, so this module adds no second copy and no
 * persisted coordinates.
 * It answers two questions and nothing else: has a contact become a drag,
 * and which of the nine did the drag land on.
 *
 ...
 * inset shifts edge rows/columns inward; the middle row and centre column are
 * pinned to the pane's centre and do not need it. It can change the answer
 * only for a drop already near a boundary. The landing itself never trusts
 * it - `MobileTools` measures the real box before and after the anchor changes
 * and animates the difference - so a small difference from a device safe-area
 * term costs a borderline choice and never a strip in the wrong place.
 */
 ...
 *
 * A CENTRE COLUMN has no horizontal inset: the stylesheet centres it with auto
 * margins between `left: 0` and `right: 0`, so its resting centre is the
 * pane's own centre whatever the strip's width. The middle row does the same
 * vertically between `top` and `bottom`, leaving transform free for drag and
 * header clearance.
 */
...
/**
 * Which of the nine the strip landed on: the nearest by straight-line distance
 ...
 * only comparison that answers "which of these nine is this closest to being".
 */
