...
it.each(["switch","unmount"])("rolls back provisional ink on %s before losing its undo bookkeeping",async end=>{
 const page=await mounted();try{
  const r=await page.evaluate(end=>(window as any).insertSpaceProbe(1,1,0,false,{end,verifySaved:true}),end);
  expect(r.live.dy).toBe(48);expect(r.down.ids.length).toBeGreaterThan(0);
  expect(r.after.strokes).toEqual(r.before.strokes);expect(r.after.doc).toBe(r.before.doc);
  expect(r.after.history).toBe(r.before.history);expect(r.undoResult).toBe(false);
  expect(r.persisted).toEqual(r.before.strokes);
  if(end==="switch"){expect(r.otherBefore.length).toBeGreaterThan(0);expect(r.otherAfter).toEqual(r.otherBefore);}
 }finally{await page.close();}
});

it.each(["pointerup","blur"])("preserves normal commit and undo through actual %s",async end=>{
 const page=await mounted();try{
  const r=await page.evaluate(end=>(window as any).insertSpaceProbe(1,1,0,false,{end,verifySaved:true}),end);
  expect(r.live.dy).toBe(48);expect(r.after.history).toBe(r.before.history+1);
  expect(r.undoResult).toBe(true);expect(r.undone.doc).toBe(r.before.doc);expect(r.undone.strokes).toEqual(r.before.strokes);
  expect(r.redone.strokes).toEqual(r.after.strokes);expect(r.persisted).toEqual(r.after.strokes);
 }finally{await page.close();}
});

it.each([.1,.5,1])("moves the right lower word independently at zoom %s",async zoom=>{
 const page=await mounted();try{
  const r=await page.evaluate(zoom=>(window as any).insertSpaceProbe(zoom,1,0,false,{shortText:"",localColumns:true,contactY:130,dy:30,moves:[20,-10,50]}),zoom);
  expect(r.down.ids).toEqual(["right-lower"]);
  expect(r.down.cut).toBeCloseTo(130,6);
  expect(r.hoverFeedback.rendered.filter((s:any)=>s.moving).map((s:any)=>s.id)).toEqual(["right-lower"]);
  expect(r.hoverFeedback.rendered.filter((s:any)=>!s.moving).map((s:any)=>s.id)).toEqual(["left-crossing"]);
  expect(r.hoverFeedback.rendered.every((s:any)=>s.alpha)).toBe(true);
  expect(r.hover).toBeCloseTo(r.contact.y,1);expect(r.live.reticle).toBeCloseTo(r.contact.y+30*r.scale,1);
  for(const move of [...r.moves,{dy:30,strokes:r.after.strokes,ids:r.down.ids}]){
   expect(move.ids).toEqual(["right-lower"]);
   for(let i=0;i<r.before.strokes.length;i++){
    const before=r.before.strokes[i],after=move.strokes[i];
    expect(after.id).toBe(before.id);
    expect(after.points).toEqual(before.points.map((p:any)=>({...p,y:p.y+(before.id==="right-lower"?move.dy:0)})));
   }
  }
  expect(r.after.doc).toBe("");expect(r.after.history).toBe(r.before.history+1);
  expect(r.undone.strokes).toEqual(r.before.strokes);expect(r.redone.strokes).toEqual(r.after.strokes);
 }finally{await page.close();}
});

it("a literal click without drag remains a no-op even with a valid moving word",async()=>{
 const page=await mounted();try{
  const r=await page.evaluate(()=>(window as any).insertSpaceProbe(1,1,0,false,{shortText:"",localColumns:true,contactY:130,clickOnly:true}));
  expect(r.down.ids).toEqual(["right-lower"]);
  expect(r.after.strokes).toEqual(r.before.strokes);expect(r.after.doc).toBe(r.before.doc);
  expect(r.after.history).toBe(r.before.history);expect(r.ended.rendered).toEqual([]);
 }finally{await page.close();}
});
...
// Distant strokes deliberately cease to share the tall stroke's membership.
// Its own midpoint is120; every stroke remains whole through drag/reversal.
it.each([119,120,121])("keeps a tall stroke whole while distant ink stays independent at contact %s",async contactY=>{
 const page=await mounted();try{
  const r=await page.evaluate(contactY=>(window as any).insertSpaceProbe(1,1,0,false,{shortText:"",tall:true,contactY,sweep:[118,119,120,121,122],moves:[70,-30,5],dy:13}),contactY);
  const expected=contactY<120?["tall","distant","below"]:["distant","below"];
  ...
  for(const sample of r.sweep){
   ...
   expect(sample.rendered.filter((s:any)=>s.moving).map((s:any)=>s.id)).toEqual(sample.y<120?["tall","distant","below"]:["distant","below"]);
